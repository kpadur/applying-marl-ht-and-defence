...
print("Attackers' state shape is", state_size_attackers, ", number of stage actions is", n_stage_actions, ", number of cyber actions is", n_cyber_actions, " and number of disinformation actions is", n_disinfo_actions)
...
for episode in range(1, number_of_episodes + 1):
    ...
    if episode != 1 and episode % saving_freq == 0:
        ...
        attack_order_df.to_csv(os.path.join(save_path, f'ch{chapter}-exp{experiment}-{date}-{seed}-attack-order.csv'), index=False)
        count_attack_actions_df = pd.DataFrame(count_timesteps_per_stage, columns = ['recon', 'cyber', 'disinfo', 'comb', 'cyber2', 'disinfo2'])
        count_attack_actions_df.to_csv(os.path.join(save_path, f'ch{chapter}-exp{experiment}-{date}-{seed}-attack-actions-count.csv'), index=False)
    ...
    if episode != 1 and episode % vis_freq == 0:
        ...
        # Plot 6: Visualise regular agents' cumulative reward
        plt.figure()
        ...
        # Plot 7: Visualise social trust in service providers
        ...
        # Plot 8: Visualise service provider availability
        plt.figure()
        for sp in range(nProviders):
        ...
        # Plot 9.1: Visualise service request rate (%) per episode
        ...
        # Plot 9.2: Visualise opinion expression rate (%) per episode
